      ...
      <section className="container order-shell">
        ...
        <aside className="order-form-card">
          ...
          <div className="order-form-body">
            ...
            <div className="direction-buttons" role="group" aria-label="Order direction"><button type="button" aria-pressed={side === 'up'} className={side === 'up' ? 'active up-button' : ''} onClick={() => setSide('up')}><span><ArrowUp /></span><div><strong>BUY UP</strong><small>Price may rise</small></div></button><button type="button" aria-pressed={side === 'down'} className={side === 'down' ? 'active down-button' : ''} onClick={() => setSide('down')}><span><ArrowDown /></span><div><strong>BUY DOWN</strong><small>Price may fall</small></div></button></div>
            ...
            <button className="submit-order" onClick={submitOrder}><CheckCircle2 /> PLACE ORDER <span>{currency === 'INR' ? '₹' : '₮'}{Number(amount || 0).toLocaleString()}</span></button>
